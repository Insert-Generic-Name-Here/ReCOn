...
class JournalSyncing:
    def __init__(self, ssh_client_dict, localpath, remotepath, verbose=False, shallow_filecmp=True):   
        ...
        if not direxists:
            self.__scp_R(self.localpath, self.remotepath)        


    def journal_syncing(self, local_journal):
        self.__read_remote_journal()
        self.__exec_journals(local_journal)
        self.__clear_journals()
        

    def __exec_journals(self, local_journal):
        print (f'{self.root}{"@"+self.ssh_client_dict["host"]} Reading the Journals. Updating Remote Server Files...\n')
        ...
        for activity in self.__journal(local_journal):
            # activity : ['timestamp' ,'event' , 'src', 'dest']
            src_path = activity[2]
            if activity[1] == 'moved':
                dest_path = activity[3]
                
                print (f"{activity[1]} --- {os.path.join(self.remotepath, self.root, ''.join(src_path.split(self.root, 1)[1:]).strip('/'))} --- {os.path.join(self.remotepath, self.root, ''.join(dest_path.split(self.root, 1)[1:]).strip('/'))}")
    ...
    def __read_remote_journal(self):     
        try:
    ...
    def __filter_journals(self, local_journal):
        concatenated_journals = pd.concat([self.remote_journal, local_journal], ignore_index=True)
    ...
    def __journal(self, local_journal):
        journal = self.__filter_journals(local_journal)
    ...
    def __scp_R(self, localpath, remotepath):
        ...
        for walker in os.walk(self.root):
            ...
            for file in walker[2]:
                if self.verbose: print (f'\t{self.root}{"@"+self.ssh_client_dict["host"]} {self.__colorize("Copying", "g")} {os.path.join(walker[0],file)}...')
